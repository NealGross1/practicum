...
if __name__ == "__main__":
    out_file = 'openai_results_aug.csv'
    used_img_ids = []
    ...
    val_path = 'E:/datasets/DOTA_dataset_512/plane_ship_aug_val_coco.json'
    ...
    aug_df = val_annots_df
    print(f"found {len(aug_df['image_id'].unique())} images")
    #'''
    data_list = []
    if not os.path.exists(out_file): 
        df = pd.DataFrame(columns=['prompt', 'reply', 'time', 'img_id', 'img_path'])
    
    for img_id in tqdm(list(aug_df['image_id'].unique())):
